...
def getRulePreparedForDb(_rule, _package_name, _access_layer_name, _section, _objs_dict):        
  log.debug("rule:")                
  pprint.pprint(_rule)         
  ...
  rule_prepared_db["policy_vendor"]=conf_d["vendor_name"]
  rule_prepared_db["policy_mgmt"]=conf_d["mgmt_name"]
    ...
    for rule in _rulebase_json["rulebase"]:  
        ...
        if "rulebase" in rule:           
            ...
            for rule in rule["rulebase"]:       
                rule_prepared_for_db_d=getRulePreparedForDb(rule, _package_name, access_layer_name, section, objects_dict)                                    
...
def import_data_into_db_main(_conf_d, _dbCon, _QLabel_status):  
    ...
    _conf_d["vendor_name"]=g_vendor_name
    ...
    for domain in domains_l:
      ...
      res_d["domains"].append({domain : packages_imp_res_d})
      ...
      for package in packages_d["packages"]:
        ...
        res_layers_l=[]
        packages_imp_res_d[package["name"]]={}
        packages_imp_res_d[package["name"]]["layers"]=res_layers_l 
      ...
      #res_d["all_imports_ok"]=getAllImportsStatus(res_d)
      # all_exports_ok=True
